...
### CI 部署 role（GitHub OIDC）— seed job 需求

CI 的 `seed-db` job（`.github/workflows/ci.yml`）以 **in-VPC ECS run-task** 方式跑一次性 seed
（複用 `igs-backend` task def、override `command=/app/seed` + `SEED_RESET=true`）。OIDC role
`igs-github-deploy-role` 除了既有的部署權限外，**還需** `ecs:RunTask`、`ecs:DescribeTasks` 與
對 `jko_ecs` 的 `iam:PassRole`。policy 已存於 [`iam-github-deploy-seed-policy.json`](./iam-github-deploy-seed-policy.json)：

```bash
aws iam put-role-policy \
  --role-name igs-github-deploy-role \
  --policy-name github-ecs-runtask-seed \
  --policy-document file://.aws/iam-github-deploy-seed-policy.json
```

> 未套用前，`seed-db` job 會在 run-task 階段以 `AccessDeniedException (ecs:RunTask)` 失敗。
