...
> **Domain 端點**（不在本 Auth 清單，各自於 domain 規格定義並已落地 `schema/openapi.yaml`）：
> - 儲值紀錄 5 個：`POST/GET /cms/deposit-records`、`GET/PATCH /cms/deposit-records/{id}`、`GET /me/deposit-records`（見 `deposit-records-api.md`）。
> - CMS Users 5 個：`GET /cms/users`、`GET/PATCH/DELETE /cms/users/{id}`、`PATCH /cms/users/me`（見 `cms-users-api.md`）。
