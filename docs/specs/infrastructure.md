...
type FamilyStore interface {
    ...
    // 由 NewFamilyStore() constructor 在啟動時呼叫；失敗則 constructor 直接回 error，
    // main 應 fatal 退出，避免冷啟動首次 refresh 才踩到 NOSCRIPT 重試 latency。
    PreloadScripts(ctx context.Context) error

    // ScriptsLoaded 回報 PreloadScripts 是否已成功完成，供 /health/ready 探測（§11.3）。
    // 在 PreloadScripts 成功前回 false；成功後恆為 true（process 生命週期內不重置）。
    ScriptsLoaded() bool
}
```
...
func (h *HealthHandler) Ready(c *gin.Context) {
    ...
    // FamilyStore Lua scripts 必須在啟動時預載；未載入代表 PreloadScripts 失敗
...
> **預載時機**：`pkg/redis/family_store.go` 的 `NewFamilyStore(client)` 在 constructor 內呼叫 `PreloadScripts()`（介面定義見 §7.4），逐一以 `SCRIPT LOAD` 寫入 Redis script cache，成功後將 `ScriptsLoaded` 旗標翻為 `true`。任一支載入失敗整個 constructor 回 error，main 直接 fatal 退出（避免冷啟動首次 refresh 才踩到 `NOSCRIPT` 重試 latency）。
...
```go
// config.LogConfig 補欄位：
type LogConfig struct {
    Level     string `mapstructure:"LOG_LEVEL" validate:"oneof=debug info warn error"`
    Format    string `mapstructure:"LOG_FORMAT" validate:"oneof=json console"`
    Service   string `mapstructure:"LOG_SERVICE"`
    AuditPath string `mapstructure:"LOG_AUDIT_PATH"` // 空 → stdout；非空 → 該檔案
}
```
